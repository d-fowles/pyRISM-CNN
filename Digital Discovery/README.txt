...
'SFED_datasets': contains .zip files of datasets produced using GAFF and OPLS solute parameters. Both of these .zip contain the functional specific multi-solvent datasets. The single optimisation method datasets can also be found here.
'training_models_scripts': scripts for training CNN, RF and PLS models can be found here. Each model has two types of script, differentiated by whether temperature descriptors are used for training.
'pyRISM_input_files.zip': contains the solvent specific input files used by pyRISM for 1D-RISM calculations. 
...
2. The 1D-RISM functional used to generate the SFED dataset must be specified. The functional will be noted in the dataset .csv file name (e.g. 'kh', 'hnc', 'gf'). 
   To specify the functional edit the 'sfed_type' variable (CNN scripts - line 81, PLS scripts - line 34, RF scripts - line 56). 

To recreate the results we have in the paper, keep all other variables as they are. The number of resamples can be changed from 50 to another value if desired (e.g. line 84 in CNN scripts). 
