...
def build_model():
    ...
    model.add(layers.Conv1D(32,                 # Number of output filters
                     ...
                     input_shape=(161,1) # Shape of input data, in order x, y, channels
                    ))
...
for resample in range(1, no_resamples + 1):
    ...
    for i, fold in enumerate(fold_list_loop):
        ...
        X_descr_val = val_fold[[col for col in val_fold.columns if col[:5]=='X_w_']]
        temp_val = val_fold['Temp'] 
        X_pd_val = pd.concat([temp_val, X_descr_val], axis=1)
        ...
        X_pd_val_scaled = (X_pd_val - means) / sds
        ...
        X_val_cv = X_pd_val_scaled.to_numpy()
    ...
    X_descr_val = val_resample[[col for col in val_resample.columns if col[:5]=='X_w_']]
    temp_val = val_resample['Temp'] 
    X_pd_val = pd.concat([temp_val, X_descr_val], axis=1)
    ...
    X_val_resample = X_pd_val_scaled.to_numpy()
...
for resample in range(1, no_resamples + 1):
    ...
    # Plot everything
    try:
        plt.plot([], [], ' ', label='Validation set predictions')
        plt.plot([], [], ' ', label=f'$R^{2}$ : {r2}')
        plt.plot([], [], ' ', label=f'RMSD : {rmsd}')
        plt.plot([], [], ' ', label=f'Bias : {bias}')
        plt.plot([], [], ' ', label=f'SDEP : {sdep}')
