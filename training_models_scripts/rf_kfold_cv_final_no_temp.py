...
def plot_feature_importance(importance, names, model_type):
    ...
    fi_df.to_csv('RF_feature_importance.csv')
    #Sort the dataframe in order of decreasing feature importance
    fi_df.sort_values(by=['feature_importance'], ascending=False, inplace=True, ignore_index=True)
    num_features = 20
    top_twenty = fi_df.iloc[:num_features]
    #Plot
    f = plt.figure(figsize=(15,15))
    plt.bar((top_twenty['feature_names']), (top_twenty['feature_importance']))
...
n_estimators = 500 #[int(x) for x in np.linspace(start = 20, stop = 500, num = 10)]
...
for resample in range(1, no_resamples + 1):
    ...
    for i, fold in enumerate(fold_list_loop):
        ...
        X_descr_train = train_fold[[col for col in train_fold.columns if col[:5]=='X_w_']]

        X_descr_test = test_fold[[col for col in test_fold.columns if col[:5]=='X_w_']]
        ...
        # No temp
        X_train_cv = X_descr_train_scaled.to_numpy()

        X_test_cv = X_descr_test_scaled.to_numpy()
        ...
        #Feature importance
        importances_cv = rf.feature_importances_
        ...
        importances_sum_cv += importances_cv
    ...
    importances_av_cv = importances_sum_cv/no_folds
    ...
    all_preds_train_cv = all_preds_train_cv.rename(columns={'Mol': f'Mol_{i}'})
    ...
    all_preds_test_cv = all_preds_test_cv.rename(columns={'Mol': f'Mol_{i}'})
    ...
    plot_feature_importance(importances_av_cv, X_descr_train.columns, 'Random Forest')
    ...
    # Doesnt use temp as a descr, Select all columns beginning with 'X_w_' as the X data
    X_descr_train = train_resample[[col for col in train_resample.columns if col[:5]=='X_w_']]

    X_descr_test = test_resample[[col for col in test_resample.columns if col[:5]=='X_w_']]
    ...
    y_pred_train_resample = rf.predict(X_train_resample)
    y_pred_test_resample = rf.predict(X_test_resample)
    ...
    importances_sum_resample += importances_resample
...
importances_av_resample = importances_sum_resample/no_resamples
...
#Plot feature importance
plot_feature_importance(importances_av_resample, X_descr_train.columns, 'Random Forest')
